...
\item For atoms not currently in a box, the coordinates are set to $<0.00, 0.00, 0.00>$. The occupancy is commonly just set to ``1.00" and is left unused by many codes. We recycle this legacy parameter by using it to denote, in our output PDBs, the box a molecule is in (box 0  occupancy=0.00 ; box 1  occupancy=1.00)
...
\item [Parameters] Provides the name and location of the parameter file to use for the simulation.
	...
	\end{tabular}}
	\end{itemize}
\item [Coordinates] Defines the PDB file names (coordinates) and location for each box in the system.
	\begin{itemize}	
	\item Value 1: $<INTEGER>$ - Sets box number (first box is box '0').\\
	\item Value 2: $<STRING>$ - Sets the name for PDB file.\\\\
...
\item [Structures] Defines the PSF filenames (structures) for each box in the system.
	\begin{itemize}	
	\item Value 1: $<INTEGER>$ - Sets box number (first box is box `0').\\
	\item Value 2: $<STRING>$ - Sets the name for PSF file.\\\\
...
\item [RotFreq] Fractional percentage at which rigid rotation move will occur.
	...
	\end{itemize}
\item [IntraSwapFreq] Fractional percentage at which molecule will be removed from a box and inserted into the same box using configurational bias algorithm.
	\begin{itemize}
...
\item [RegrowthFreq] Fractional percentage at which part of the molecule will be deleted and then regrown using configurational bias algorithm.
	...
	\end{itemize}
\item [VolFreq] (\colorbox{cyan}{For isobaric-isothermal ensemble and Gibbs ensemble runs only}) Fractional percentage at which molecule will be removed from one box and inserted into the other box using configurational bias algorithm.
	\begin{itemize}
	\item Value 1: $<DOUBLE>$ - \% Volume swaps
	\end{itemize}
\item [SwapFreq] (\colorbox{cyan}{For Gibbs and Grand Canonical (GC) ensemble runs only}) Fractional percentage at which molecule swap move will occur.
	\begin{itemize}
...
\item [BlockAverageFreq] Controls the block averages output of selected thermodynamic properties. Block averages are averages of thermodynamic values of interest for chunks of the simulation (for post-processing of averages or std. dev. in those values).
	...
	\end{itemize}
\item [HistogramFreq] Controls the histograms. Histograms are a binned listing of observation frequency for a specific thermodynamic variable.  In this code, they also control the output of a file containing energy/molecule samples; it only will be used in GC ensemble simulations for histogram reweighting purposes.
	\begin{itemize}
...
\begin{description}
\item [DistName] Sets short phrase to naming molecule distribution file.
	\begin{itemize}
	\item Value 1: $<STRING>$ - Short phrase which will be combined with \texttt{RunNumber} and \texttt{RunLetter} to use in the name of the binned histogram for molecule distribution.
	\end{itemize}
\item [HistName] Sets short phrase to naming energy sample file.
	\begin{itemize}
	\item Value 1: $<STRING>$ - Short phrase, which will be combined with \texttt{RunNumber} and \texttt{RunLetter}, to use in the name of the energy/molecule count sample file.
	\end{itemize}
...
\colorbox{yellow}{\texttt{NOTE:}} If total energy of simulation is greater that $1.0 e^{12}$, \texttt{System Total Energy Calculation} will be performed at \texttt{EqSteps} to preserve energy value.
\begin{figure}[H]
...
\begin{figure}[H]
\includegraphics[scale=0.7]{images/out5}
\end{figure}
