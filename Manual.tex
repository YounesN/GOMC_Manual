...
\end{figure}
\newpage
\section{Intermolecular Energy and Virial function}
In this section, the virial and energy equation for different potential function are discussed in details.
...
\begin{equation}
W_{ij} = 6\times C_{n_{ij}} \epsilon_{ij} \bigg[\frac{n_{ij}}{2} \times \bigg(\frac{\sigma_{ij}}{r_{ij}}\bigg)^{n_{ij}} - \bigg(\frac{\sigma_{ij}}{r_{ij}}\bigg)^6\bigg]\times \frac{\overrightarrow{r_{ij}}}{{r_{ij}}^2}
\end{equation}
...
\begin{itemize}
	...
	\begin{equation}
E_{ij}(\texttt{shift}) = C_{n_{ij}} \epsilon_{ij} \bigg[\bigg(\frac{\sigma_{ij}}{r_{ij}}\bigg)^{n_{ij}} - \bigg(\frac{\sigma_{ij}}{r_{ij}}\bigg)^6\bigg] - C_{n_{ij}} \epsilon_{ij} \bigg[\bigg(\frac{\sigma_{ij}}{r_cut}\bigg)^{n_{ij}} - \bigg(\frac{\sigma_{ij}}{r_cut}\bigg)^6\bigg]
\end{equation}

where $r_{ij}$, $\epsilon_{ij}$, and $\sigma_{ij}$ are, respectively, the separation, well depth, and collision diameter for the pair of interaction sites $i$ and $j$. The constant $C_n$ is a normalization factor according to Eq. 2, such that the minimum of the potential remains at $-\epsilon_{ij}$ for all $n_{ij}$. In the 12-6 potential, $C_n$ reduces to the familiar value of 4.
	\item Virial Calculation: Virial is basically the negative derivative of energy with respect to distance, multiplied by distance, Eq. 3.\\Using \texttt{SHIFT} potential function defined above:
\begin{equation}
W_{ij}(\texttt{shift}) = 6\times C_{n_{ij}} \epsilon_{ij} \bigg[\frac{n_{ij}}{2} \times \bigg(\frac{\sigma_{ij}}{r_{ij}}\bigg)^{n_{ij}} - \bigg(\frac{\sigma_{ij}}{r_{ij}}\bigg)^6\bigg]\times \frac{\overrightarrow{r_{ij}}}{{r_{ij}}^2}
\end{equation}
\end{itemize}
\begin{figure}[H]
\centering
...
\begin{equation}
W_{ij}(\texttt{switch}) = 6 C_{n_{ij}} \epsilon_{ij} \bigg[\frac{n_{ij}}{2} \times \bigg(\frac{\sigma_{ij}}{r_{ij}}\bigg)^{n_{ij}} - \bigg(\frac{\sigma_{ij}}{r_{ij}}\bigg)^6\bigg]\times F_E \times  \frac{\overrightarrow{r_{ij}}}{{r_{ij}}^2} - C_{n_{ij}} \epsilon_{ij} \bigg[\bigg(\frac{\sigma_{ij}}{r_{ij}}\bigg)^{n_{ij}} - \bigg(\frac{\sigma_{ij}}{r_{ij}}\bigg)^6\bigg] \times F_W
\end{equation}
