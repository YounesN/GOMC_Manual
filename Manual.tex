...
\title{\Huge User Manual\\Version 2.10}
...
ß C++03 Compliant Compiler
ßß Linux/OS X
ßßß icpc (Intel C++ Compiler)\\\\
...
ßßß cmake (if compiling on Linux)\\
...
\underline{\textbf{Compilation on Windows}}\\
...
\underline{\textbf{Compilation on Linux}}\\
...
\texttt{\$ ./metamake.sh}\\\\
This script will create a directory and run cmake for you. Once it's complete, it will compile the code as well. Once it is done it should generate all the executables in the ``bin" directory.
...
\item \textbf{The beta value in GOMC code is used to define the mobility of the molecule. }
...
\colorbox{red}{\texttt{Warning:}} In order to make topology file for a linear molecule such as water, it needs to be sure that atoms are generated and bonded in a linear order. Otherwise CBMC algorithm would fail. However, for branch molecules, any order can be used to generate and bond the atoms of a molecule.\\
Here's an example of a residue definition for water:\\\\
\colorbox{lightgray}{
\begin{tabular}{l}
\texttt{RESI SPCE ~~ 0.00 ! SPC/E water}\\
\texttt{GROUP}\\
\texttt{ATOM H1 HO    0.4238 !  ~ O1}\\
\texttt{ATOM O1 OH   -0.8476 !   H1~H2}\\
\texttt{ATOM H2 HO    0.4238 !   }\\
\texttt{BOND H1 O1  O1 H2 }\\
\texttt{PATCHING FIRS NONE LAST NONE}
\end{tabular}}\\\\
...
\item [Coordinates] Defines the PDB filenames (coordinates) for each box in the system.
	...
	\colorbox{yellow}{\texttt{NOTE:}} In case of \texttt{Restart   true}, the restart PDB output file from GOMC (OutputName\_BOX\_0\_restart.pdb) can be used for both boxes. \\\\
...
\item [Potential] Defines the potential function type to calculate non-bonded interaction energy and force between atoms.
	...
	\item Value 1: $<STRING>$ - Allows you to pick between ``VDW'', ``SHIFT'' and ``SWITCH''.
		...
		\end{description}
	\end{itemize}
\item [Rswitch] In the case of choosing ‚ÄúSWITCH‚Äù as potential function, a distance is set in which non-bonded interaction energy is truncated smoothly from to cutoff distance.
	\begin{itemize}
...
\item [PressureCalc] Considers to calculate the pressure or not. If it is set to true, the frequency of pressure calculation need to be set.
	...
	\end{itemize}
\item [1-4scaling] Defines constant factor to modify 1-4 intra-molecule coulomb interaction.
	\begin{itemize}
...
\item [Fugacity] (\colorbox{cyan}{For Grand Canonical (GC) ensemble runs only}): Fugacity at which simulation is run.
	\begin{itemize}
	\item Value 1: $<STRING>$ - The resname to apply this fugacity w.r.t.
...
\item [BoxDim] Defines the axis lengths of simulation box. This tag may occur multiple times.  It occurs once for \texttt{NVT} and \texttt{NPT}, but twice for Gibbs ensemble or GC ensemble.
	...
	\item Value 3: $<DOUBLE>$ ‚Äì y-axis length in Angstroms
	...
	\colorbox{red}{\texttt{Warning:}} In case of \texttt{Restart   true}, box dimension must be manually entered. The box dimension can be find from the first line of restart PDB output file from GOMC (OutputName\_BOX\_0\_restart.pdb and OutputName\_BOX\_1\_restart.pdb). \\
...
\item [RestartFreq] Controls the output of the last state of simulation at a specified step in PDB files (coordinates) OutputName\_BOX\_\texttt{n}\_restart.pdb, where \texttt{n} defines the box number. If PDB dumping was enabled, one file for NVT or NPT and two files for Gibbs ensemble or GC ensemble will be dumped.
	\begin{itemize}
	\item Value 1: $<BOOLEAN>$ - ``true'' enables dumping these files; ``false'' disables dumping.
...
\item [RunNumber] Sets a number, which is a part of \texttt{DistName} and \texttt{HistName} file name.
	\begin{itemize}
	\item Value 1: $<UINT>$ ‚Äì Run number to be used in the above file names.
	\end{itemize}
\item [RunLetter] Sets a letter, which is a part of \texttt{DistName} and \texttt{HistName} file name.
	\begin{itemize}
	\item Value 1: $<CHAR>$ ‚Äì Run letter to be used in above file names.
	\end{itemize}
\item [SampleFreq] Controls histogram sampling frequency. 
	\begin{itemize}
	\item Value 1: $<UINT>$ ‚Äì the number of steps per histogram sample.
	\end{itemize}
	\colorbox{lightgray}{
	\begin{tabular}{l}
	\texttt{\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#}\\
...
(\textcolor{orange}{*}) = NVT ensemble, (\textcolor{blue}{*}) = NPT ensemble and Gibbs ensemble, (\textcolor{green}{*}) = GC ensemble
	\begin{itemize}
	\item Value 1: $<BOOLEAN>$ ‚Äì ``true'' enables message output of block averages via this tracked parameter (and in some cases such as entry, components); ``false'' disables it.
	\item Value 2: $<BOOLEAN>$ ‚Äì ``true'' enables message output of a fluctuation into the console file via this tracked parameter (and in some cases, such as entry, components); ``false'' disables it.
	\end{itemize}
	\colorbox{lightgray}{
	\begin{tabular}{l}
	\texttt{\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#}\\
...
\colorbox{yellow}{\texttt{NOTE:}} In order to keep the format of console file consistent, if calculated energy of simulation is greater that 9999999999.9999, the value of 999999999 will be printed instead.\\
...
\colorbox{yellow}{\texttt{NOTE:}} In order to keep the format of Blockaverage file consistent, if calculated energy of simulation is greater that 9999999999.9999, the value of 999999999 will be printed instead.\\
...
\texttt{MASS   2  CH1    13.019 C  !}\\\\
\texttt{AUTOGENERATE ANGLES DIHEDRALS}\\\\
\texttt{RESI ISB        0.00 ! isobutane ‚Äì TraPPE}\\
\texttt{GROUP}\\
\texttt{ATOM C1 CH1     0.00 !      C3}\\
\texttt{ATOM C2 CH3     0.00 ! C2-C1}\\
\texttt{ATOM C3 CH3     0.00 !      C4}\\
