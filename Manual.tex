...
\begin{align*}
U_{bond}&=\sum_{bonds} K_b(b-b^0)^2 & U_{LJ} &=\sum_{nonb,pairs} \epsilon_{ij}\left[\left(\frac{r_{ij}^{min}}{r_{ij}}\right)^{12}-2\left(\frac{r_{ij}^{min}}{r_{ij}}\right)^6\right] \\
U_{angle}&=\sum_{angles} K_{\theta}(\theta-\theta^0)^2 & U_{LJ} &=\sum_{elec} \frac{q_i q_j}{4\pi \epsilon r_{ij}} \\
U_{dihedral}&=\sum_{dihedrals} K_{\phi} (1+\cos(n\phi - \delta))
\end{align*}
